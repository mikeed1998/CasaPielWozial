	...
	<div class="uk-width-1-1">
		<div class="uk-card uk-card-default uk-card-body">
			...
			while ($rowCONSULTA = $CONSULTA -> fetch_assoc()) {
				...
				//insertar registros para color
				$CONSULTA4 = $CONEXION -> query("SELECT * FROM productosexistencias WHERE producto = $id AND talla = $tallaId");
					$numItems=$CONSULTA4->num_rows;
					if ($numItems==0) {
						$sql="INSERT INTO productosexistencias (producto,talla) VALUES ($id,$tallaId)";
						if ($insertar = $CONEXION->query($sql)) {
						...
						$CONSULTA3 = $CONEXION -> query("SELECT * FROM productosexistencias WHERE producto = $id AND talla = $tallaId");
				...
				<li>
					...
					<div uk-grid class="uk-child-width-1-1@xl uk-child-width-1-1@m">
					';
						...
						<div>
							<div class="uk-card uk-card-default uk-card-body">
								...
								<div>
									...
									<select name="categoria" data-placeholder="Seleccione una" class="editarajax chosen-select uk-select" data-tabla="productosexistencias" data-campo="color" data-id="'.$existenciasId.'">';
										// Obtener todos los colores
										echo'<option data-tabla="productosexistencias">Selecciona un color</option>';
										...
										while ($rowCONSULTA2 = $CONSULTA2 -> fetch_assoc()) {
											...
											if(isset($color) && $color == $colorId){
												$estatus='selected';
											}else{
												$estatus='';
											}

											echo'<option class="editarajax" data-tabla="productosexistencias" data-campo="color" data-id="'.$existenciasId.'" value="'.$colorId.'" '.$estatus.'>'.$rowCONSULTA2['name'].'</option>';
										}
										echo'	
										</select>
									</div>
