		...
		switch ($enviarcorreo) {
			...
			case 2:
				...
				$cuerpoCorreo = '
					...
					'.$remitenteMail.'<br><br>
					Si desea ver su pedido puede hacerlo en el suiguiente enlace:<br><br><br><br>
					<a href="'.$ruta.'mi-cuenta" style="background-color:'.$mailButton.';font-weight:700;border-radius:8px;padding-left:30px;padding-right:30px;padding-top:10px;padding-bottom:10px;color:white;text-decoration:none;">Mi cuenta</a><br><br><br><br>
					o copie y pegue este enlace: <br>
					<a href="'.$ruta.'mi-cuenta">'.$ruta.'mi-cuenta</a><br><br>
					Saludos cordiales.';
	...
	if (isset($_POST['facebooklogin'])) {
		...
		if (strlen($facebookId)>0) {
			...
			if ($numUser==0) {
				...
				$sql = "INSERT INTO usuarios (nombre,email,alta)".
					"VALUES ('$nombre','$email','$hoy')";
...
//    Subir comprobante                  
	if(isset($_GET['comprobantefile'])){

		$id=$_GET['id'];
		$imagenName=$_GET['comprobantefile'];
		...
		if(file_exists($rutaInicial.$imagenName)){
			...
			header('location: ../es/mi-cuenta'.$estatus);
		}else{
...
//    Cambiar foto de perfil             
	...
	//	  Ordenar resultados				 
	if (isset($_POST['odenarpor'])) {
		$_SESSION['odenarpor']=$_POST['odenarpor'];
		$xtras=$_SESSION['odenarpor'];
	}
...
// Mostrar mensaje
	echo '{ "msj":"<div class=\'uk-text-center color-blanco bg-'.$msjClase.' padding-10 text-lg\'><i uk-icon=\'icon:'.$msjIcon.';ratio:3;\'></i> &nbsp; '.$msjTxt.'</div>", "estatus":"'.$fallo.'", "xtras":"'.$xtras.'" '.$estatus.''.$horario.'}';
